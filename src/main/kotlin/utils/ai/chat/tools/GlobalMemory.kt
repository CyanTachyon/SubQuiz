...
object GlobalMemory: KoinComponent, AiToolSet.ToolProvider
{
    override val name: String get() = "写入全局记忆"
    ...
    override suspend fun AiToolSet.registerTools()
    {
        ...
        {
            if (!parm.value.isNullOrBlank()) Content("存储条目: `${parm.key}` = `${parm.value}`")
            else Content("删除条目: `${parm.key}`")
